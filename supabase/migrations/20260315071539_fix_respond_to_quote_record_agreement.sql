/*
  # Fix Quote Accept: Record Service Agreement Inside RPC

  ## Summary
  The agreement_accepted field was being updated by the frontend via a direct
  table update which fails for anon/guest users due to RLS. Moving the
  agreement recording into the respond_to_quote_by_token function ensures it
  is always captured regardless of authentication state.

  The frontend's direct supabase.update() call is now redundant (agreement is
  recorded inside the RPC), but it remains harmless for authenticated users.

  ## Changes
  - respond_to_quote_by_token: When action='accept', also set
    agreement_accepted=true and agreement_accepted_at=now() on the quote.
*/
