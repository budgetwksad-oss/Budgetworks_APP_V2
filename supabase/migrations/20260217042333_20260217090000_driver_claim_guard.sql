/*
  # Enforce Driver Eligibility for Claims

  1. Security Enhancement
    - Update claim_job_position RPC to enforce can_drive requirement
    - Only crew members with profiles.can_drive=true can claim driver roles
    - Raise exception if non-driver attempts to claim driver position

  2. Changes
    - Replace claim_job_position function with enhanced version
    - Add can_drive validation when p_role='driver'
    - Keep all existing race-condition protection and capacity checks

  3. Error Handling
    - Raise 'Driver role requires can_drive=true' for ineligible crew
    - All other validation logic remains unchanged

  4. Notes
    - Helper roles unaffected
    - Admin assignment functionality not impacted
    - Existing claims remain valid
*/

-- Replace claim_job_position with enhanced driver validation
...
BEGIN
  ...
  -- Check user is crew member and get can_drive status
  ...
  -- Enforce driver eligibility
  IF p_role = 'driver' AND NOT v_can_drive THEN
    RAISE EXCEPTION 'Driver role requires can_drive=true';
  END IF;
...
-- Update comment
COMMENT ON FUNCTION claim_job_position IS 'Atomically claim a job position as a crew member. Validates capacity, driver eligibility, and prevents race conditions.';
